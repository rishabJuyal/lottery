            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="relative flex-1 p-2 flex flex-col overflow-y-auto pb-28 scrollbar-thin scrollbar-thumb-[var(--color-primary-light)] scrollbar-track-blue-[var(--color-primary)]">
                    <nav className="flex flex-col space-y-2 sm:space-y-3 text-sm sm:text-base">
                        {sidebarLinks.map((link, index) => (
                            <Link
                                ...
                                className="flex items-center justify-between hover:bg-[var(--color-primary-light)] p-2.5 rounded-lg transition-all"
                            >
                    ...
                    <div className="absolute bottom-0 left-0 w-full">
                        ...
                        <div
                            className={`bg-[var(--color-primary-dark)] overflow-hidden transition-all duration-300 ease-in-out ${aboutOpen ? "max-h-52 opacity-100" : "max-h-0 opacity-0"
                                }`}
                        >
                            {["Terms and Conditions", "Privacy Statement", "Contact Us"].map(
                                (text, idx) => (
                                    <Link
                                        to={`/${text.toLowerCase().replace(/\s+/g, "-")}`} // optional routing
                                        key={idx}
                                        onClick={onClose}
                                        className="block px-5 py-2 text-xs sm:text-sm text-gray-200 hover:bg-[var(--color-primary-light)]"
                                    >
                                        {text}
                                    </Link>
                                )
                    ...
                    {bottomNavData.map((btn, index) => {
                        ...
                        return (
                            <Link
                                to={btn.path}
                                key={index}
                                onClick={onClose} // Close sidebar on right nav click
                                className="w-full flex justify-center"
                            >
                                <RightButton icon={<Icon size={32} />} label={btn.label} />
                            </Link>
                        );
                    })}
                </div>
...
const RightButton = ({ icon, label }) => (
    <button className="flex flex-col items-center text-[9px] sm:text-[10px] font-semibold text-center leading-tight hover:scale-105 transition-transform">
        {icon}
